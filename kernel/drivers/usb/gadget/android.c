			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
		 ...
		 * device descriptor.
		 */
		 //wisky-lxh@20130527,read VID from projectconfig.mk
		 #ifdef WISKY_USB_VENDOR_ID
		device_desc.idVendor = simple_strtoul(WISKY_USB_VENDOR_ID, NULL, 0);
		//printk("--->usb vendor=0x%x\n",);
		#endif
		#ifdef WISKY_USB_PRODUCT_ID
		device_desc.idProduct = simple_strtoul(WISKY_USB_PRODUCT_ID, NULL, 0);
		//printk("--->usb vendor=0x%x\n",);
		#endif
		//end-wisky-lxh
